...
if __name__ == '__main__':
    ...
    image_files = sorted(glob.glob(phone + '/*'))
    
    plt.ion()
    cmap = 'binary'
    fig, ax = plt.subplots(3, 3)
    for i, image_file in enumerate(image_files[350:]):
        if DEBUG_IMAGE:
            image = array.array('f')
            ...
            def rgb2gray(rgb):
                ...
                r, g, b = 0.2989 * r, 0.5870 * g, 0.1140 * b
                gray = r + g + b
                return gray, r, g, b

            diff = image[:,:,6:9] - image[:, :, 3:6] - image[:, :, 0:3]
            gray, r, g, b = rgb2gray(diff)

            ax[0, 0].imshow(image[:,:,0:3])
            ax[0, 1].imshow(image[:,:,3:6])
            ax[0, 2].imshow(image[:,:,6:9])
            ax[1, 0].imshow(r, cmap=cmap)
            ax[1, 1].imshow(g, cmap=cmap)
            ax[1, 2].imshow(b, cmap=cmap)
            ax[2, 0].imshow(diff)
            ax[2, 1].imshow(gray, cmap=cmap)
